class KioskMode {
  constructor() {
    ...
    this.run();
    this.entityWatch();
    new MutationObserver(this.watchDashboards).observe(
  ...
  processConfig(lovelace, config) {
    ...
    if (queryStringsSet) {
      this.hideHeader =
        this.cached("kmHeader") || this.queryString(["kiosk", "hide_header"]);
      this.hideSidebar =
        this.cached("kmSidebar") || this.queryString(["kiosk", "hide_sidebar"]);
      this.hideOverflow =
        this.cached("kmOverflow") ||
        this.queryString(["kiosk", "hide_overflow"]);
      this.hideMenuButton =
    ...
    if (config.user_settings) {
      for (let conf of this.array(config.user_settings)) {
        if (
          this.array(conf.users).some(
            (x) => x.toLowerCase() == this.user.name.toLowerCase()
          )
        )
  ...
  watchDashboards(mutations) {
    mutations.forEach(({ addedNodes }) => {
      for (let node of addedNodes)
        if (node.localName == "ha-panel-lovelace") window.KioskMode.run(node);
    });
  }
